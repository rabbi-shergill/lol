...
# SETUP
# input_tensors = [tf.placeholder(tf.float32) for _ in range((28 // config.window_size()) ** 2)]	# time_step x batch_size x input_dims
...
images = np.concatenate((images, np.zeros(list(np.shape(images)[: -1]) + [2])), axis = -1)
t_images = np.concatenate((t_images, np.zeros(list(np.shape(t_images)[: -1]) + [2])), axis = -1)
print('Data Loaded\t\t\t')
...
def train(epoch, global_step):
	# optimizer = config.optimizer(steps = global_step, epoch = epoch).minimize(loss)
